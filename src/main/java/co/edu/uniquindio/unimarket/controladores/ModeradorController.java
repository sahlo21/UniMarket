...
public class ModeradorController {
    ...
    @GetMapping("/listarProducto/{codigoProducto}")
    public ResponseEntity<MensajeDTO> listarProductos(@PathVariable int codigoProducto){
        return ResponseEntity.status(HttpStatus.OK).body(new MensajeDTO(HttpStatus.OK, false, moderadorServicio.listarProductos(codigoProducto)));
    }
    @PutMapping("/revisarProducto/{codigoProducto}")
    public ResponseEntity<MensajeDTO> revisarProducto (@PathVariable int codigoProducto){
